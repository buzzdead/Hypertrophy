...
export const ProgressTracking = () => {
  ...
  const getChartData = async (lh?: boolean, cm?: number) => {
    ...
    const newExercises = state.mode === 'Daily' ? await fetchExercises({by: "Month", when: availableMonths[newCurrentMonth]?.numerical}) : await fetchExercises()
  ...
  const getChartData2 = async (selectedCategories: CategorySchema[]) => {
    ...
    const newExercises = state.mode === 'Daily' ? await fetchExercises({by: "Month", when: state.availableMonths[state.currentMonth]?.numerical}) : await fetchExercises()
    const {chartData, maxExercises, days} = ChartData({
      exercises: newExercises,
  ...
  return (
    <SafeAreaView style={{width: "100%", height: "100%", justifyContent: "center", alignItems: "center", gap: 20}}>
      <Chart isLandScape={screenOrientation.isLandscape} mode={state.mode} maxExercises={state.maxExercises} chartData={state.chartData} days={state.days} />
      ...
      <ProgressTrackingBtm mode={state.mode} landScapeOrientation={screenOrientation.isLandscape} changeMode={(newMode) => setState({...state, mode: newMode})} />
